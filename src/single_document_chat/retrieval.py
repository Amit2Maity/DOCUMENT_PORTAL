...
class ConversationalRAG:
    def __init__(self,session_id: str, retriever) -> None:
        ...
        self.sessin_id = session_id
        self.retriever = retriever
        try:
           self.llm = self._load_llm()
           self.contextualize_prompt = PROMPT_REGISTRY[PromptType.CONTEXTUALIZE_QUESTION.value]
           self.qa_promt = PROMPT_REGISTRY[PromptType.CONTEXT_QA.value]
           
           self.history_aware_retriever = create_history_aware_retriever(
               self.llm, self.retriever, self.contextualize_prompt
              )
           self.log.info("History-aware retriever created successfully", session_id=session_id)
           
           self.qa_chain = create_stuff_documents_chain(self.llm, self.qa_promt)
           self.rag_chain = create_retrieval_chain(self.history_aware_retriever, self.qa_chain)
           self.log.info("RAG chain created successfully", session_id=session_id)
    ...
    def invoke(self, user_input:str):
        try:
            ...
            if not answer:
                self.log.warning("No answer returned from RAG chain", session_id=self.session_id)
